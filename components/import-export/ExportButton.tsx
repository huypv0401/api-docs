...
const FORMATS = [
  { label: 'Export Postman', endpoint: (id: string) => `/api/export/postman/${id}`, ext: 'postman_collection.json' },
  { label: 'Export OpenAPI', endpoint: (id: string) => `/api/export/openapi/${id}`, ext: 'openapi.json' },
]

export function ExportButton({ documentId, title }: { documentId: string; title: string }) {
  ...
  const handleExport = async (fmt: (typeof FORMATS)[number]) => {
    ...
    try {
      const res = await fetch(fmt.endpoint(documentId))
